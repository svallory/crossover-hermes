...
class MockRunnableLLMInquiry(RunnableSerializable):
    ...
    name: str = "MockRunnableLLMForAdvisor"
...
class TestAdvisor(unittest.TestCase):
    ...
    @patch("src.hermes.agents.advisor.search_products_by_description")
    @patch("src.hermes.tools.response_tools.extract_questions")
    @patch("src.hermes.agents.advisor.resolve_product_reference")
    @patch("src.hermes.agents.advisor.answer_product_question")
    @patch("src.hermes.agents.advisor.generate_response_points")
    @patch("src.hermes.agents.advisor.get_llm_client")
    @patch("src.hermes.agents.advisor.find_related_products")
    def test_seasonal_inquiry(
        ...
        async def generate_points_side_effect(
            primary_products,
            answered_questions,
            related_products,
        ...
        email_analysis_obj = EmailAnalysis(
            ...
            customer_signals=[
                CustomerSignal(
                    signal_type="seasonal_preference",
                    signal_category="Context",
                    signal_text="for spring",
                    signal_strength=0.8,
                    excerpt="for spring",
                )
            ],
        ...
        self.assertIn(
            "This product is great for spring.",
            inquiry_result_dict.get("response_points", []),
        )
    ...
    def test_occasion_specific_inquiry(
        ...
        mock_q_answer = QuestionAnswer(
            question="Is this good for a wedding?",
            answer="Yes, it is perfect.",
            confidence=0.9,
            relevant_product_ids=[transformed_product_fixture["product_id"]],
        )

        async def answer_side_effect_occasion(question, primary_products, product_catalog_df, vector_store, llm):
            return {
                "is_answered": True,
                "answer_text": mock_q_answer.answer,
                "confidence": mock_q_answer.confidence,
                "product_ids": mock_q_answer.relevant_product_ids,
            }
        ...
        async def generate_points_side_effect_occasion(
            primary_products,
            answered_questions,
            related_products,
            unanswered_questions,
        ...
        email_analysis_obj = EmailAnalysis(
            ...
            product_references=[
                ProductReference(
                    ...
                    quantity=0,
                    confidence=0.8,
                    excerpt="dress for a summer wedding",
                )
            ],
            customer_signals=[
                CustomerSignal(
                    signal_type="occasion",
                    signal_category="Context",
                    signal_text="summer wedding",
                    signal_strength=0.9,
                    excerpt="summer wedding",
                )
            ],
            classification_confidence=0.9,
    ...
    def test_material_quality_inquiry(
        ...
        # Mock answer_product_question
        mock_q_answer_material = QuestionAnswer(
            question="Is the material good?",
        ...
        async def answer_side_effect_material(question, primary_products, product_catalog_df, vector_store, llm):
            return {
                "is_answered": True,
                "answer_text": mock_q_answer_material.answer,
                "confidence": mock_q_answer_material.confidence,
                "product_ids": mock_q_answer_material.relevant_product_ids,
            }
        ...
        # Mock generate_response_points
        async def generate_points_side_effect_material(
            primary_products,
            answered_questions,
            related_products,
            unanswered_questions,
        ...
        email_analysis_obj = EmailAnalysis(
            ...
            customer_signals=[
                CustomerSignal(
                    ...
                    signal_text="is the material good enough",
                    signal_strength=0.9,
                    excerpt="material good enough",
                )
            ],
            classification_confidence=0.9,
    ...
    def test_style_inspiration_inquiry(
        ...
        # Mock answer_product_question
        mock_q_answer = QuestionAnswer(
            question="What era are these inspired by?",
            answer="1960s mod era",
            confidence=0.9,
            relevant_product_ids=[jean_product_fixture["product_id"]],
        )
        ...
        # Mock generate_response_points
        async def generate_points_side_effect_style(
            primary_products,
            answered_questions,
            related_products,
            unanswered_questions,
    ...
    def test_product_not_found_resolution(
        ...
        email_analysis_obj = EmailAnalysis(
            ...
            product_references=[
                ProductReference(
                    ...
                    quantity=0,
                    confidence=0.9,
                    excerpt=f"looking for {product_ref_text}",
                )
            ],
            classification_confidence=0.9,
            ...
            tone_analysis={"tone": "neutral", "formality_level": 3, "key_phrases": []},
            customer_signals=[],
            reasoning="Mocked for not found test",
        )
        ...
        self.assertTrue(
            any(unsuccessful_ref == product_ref_text for unsuccessful_ref in inquiry_result.unsuccessful_references),
    ...
    def test_multi_language_inquiry(
        ...
        mock_q_answer_lang = QuestionAnswer(
            question="¿De qué material está hecho?",
            answer="Es de lana y acrílico.",
            confidence=0.9,
            relevant_product_ids=[transformed_product_fixture["product_id"]],
        )
        ...
        async def answer_side_effect_lang(question, primary_products, product_catalog_df, vector_store, llm):
            return {
                "is_answered": True,
                "answer_text": mock_q_answer_lang.answer,
                "confidence": mock_q_answer_lang.confidence,
                "product_ids": mock_q_answer_lang.relevant_product_ids,
            }
        ...
        # Mock generate_response_points
        async def generate_points_side_effect_lang(
            primary_products,
            answered_questions,
            related_products,
            unanswered_questions,
        ...
        email_analysis_obj = EmailAnalysis(
            ...
            product_references=[
                ProductReference(
                    ...
                    quantity=0,
                    confidence=0.9,
                    excerpt="pregunta sobre el CHN0987",
                )
            ],
            customer_signals=[],
