...
class TestOrderProcessor(unittest.TestCase):
    ...
    def test_last_item_stock(
        ...
        email_analysis_obj.product_references = [
            ProductReference(
                reference_text="RSG8901 Retro Sunglasses", reference_type="product_id",
                product_id="RSG8901", product_name="Retro Sunglasses", quantity=1,
                confidence=0.95, excerpt="I would like to order 1 pair of RSG8901 Retro Sunglasses"
            )
        ]
        ...
        async def resolve_side_effect(ref, catalog_df, llm):
            return Product(
                product_id="RSG8901", name="Retro Sunglasses", category="Accessories",
                stock_amount=1, description="Cool shades", price=product_data.get('price', 25.00)
            )
    ...
    def test_exceeds_stock(
        ...
        email_analysis_obj.product_references=[
            ProductReference(
                reference_text="retro sun glasses (RSG8901)", reference_type="product_id",
                product_id="RSG8901", product_name="Retro Sunglasses", quantity=2,
                confidence=0.95, excerpt="I'd like to buy 2 pairs of the retro sun glasses (RSG8901)"
            )
        ]
    ...
    def test_complex_format_reference(
        ...
        email_analysis_obj.product_references=[
            ProductReference(
                reference_text="Chelsea Boots [CBT 89 01]", reference_type="product_id", # Will be normalized
                product_id="CBT8901", product_name="Chelsea Boots", quantity=1,
                confidence=0.9, excerpt="I would like to buy Chelsea Boots [CBT 89 01]"
            )
        ]
        ...
        async def resolve_side_effect(ref, catalog_df, llm):
            return Product(
                product_id="CBT8901", name="Chelsea Boots", category="Footwear",
                stock_amount=product_data.get('stock amount', 5), description="Stylish boots", price=product_data.get('price', 75.00)
            )
        mock_resolve_reference.side_effect = resolve_side_effect
    ...
    def test_multiple_item_order(
        ...
        email_analysis_obj.product_references=[
            ProductReference(
                reference_text="cable knit beanies", reference_type="product_id", product_id="CLF2109", 
                product_name="Cable Knit Beanie", quantity=5, confidence=0.9, excerpt="5 cable knit beanies"
            ),
            ProductReference(
                reference_text="fuzzy slippers", reference_type="product_id", product_id="FZZ1098",
                product_name="Fuzzy Slippers", quantity=2, confidence=0.9, excerpt="2 pairs of fuzzy slippers"
            )
        ]
        ...
        mock_update_stock.assert_any_call(product_id="CLF2109", quantity_to_decrement=1, product_catalog_df=state.product_catalog_df)
        mock_update_stock.assert_any_call(product_id="FZZ1098", quantity_to_decrement=2, product_catalog_df=state.product_catalog_df)
    ...
    def test_promotion_detection(
        ...
        email_analysis_obj.product_references=[
            ProductReference(
                reference_text="those amazing bags", reference_type="product_name", # Name based reference
                product_name="Canvas Beach Bag", quantity=1, # Assuming 1 for simplicity here, email is vague
                confidence=0.85, excerpt="those amazing bags with the summer vibe"
            )
        ]
        email_analysis_obj.reasoning = "Order with potential promotion."
        email_analysis_obj.customer_signals = [
            {"signal_type": "promotion_awareness", "signal_text": "read about your BOGO offer", "signal_strength": 0.8}
        ]
        ...
        async def resolve_side_effect(ref, catalog_df, llm):
            return Product(
                product_id="CBG9876", name="Canvas Beach Bag", category="Accessories",
                stock_amount=20, description="Large beach bag", price=product_data.get('price', 20.00)
            )
        mock_resolve_reference.side_effect = resolve_side_effect
        ...
        mock_extract_promotion.assert_called_once_with(
            product_description="Large beach bag", 
            product_name="Canvas Beach Bag"
        )
        mock_update_stock.assert_called_once()
    ...
    def _transform_fixture_to_product_input(self, fixture_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'product_id' not in transformed: transformed['product_id'] = "DEFAULT_ID"
        if 'stock_amount' not in transformed: transformed['stock_amount'] = 0
        if 'price' not in transformed: transformed['price'] = 0.0
