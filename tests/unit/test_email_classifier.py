...
class TestEmailClassifier(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_mixed_intent_email(self, mock_get_llm_client, mock_verify_analysis, mock_openai_client):
        ...
        future_intent = next((s for s in analysis["customer_signals"] 
                              if s["signal_type"] == "future_purchase_intent"), None)
    ...
    async def test_vague_reference_email(self, mock_get_llm_client, mock_verify_analysis, mock_openai_client):
        ...
        mock_llm_output = EmailAnalysis(
            ...
            tone_analysis={
                "tone": "curious",
                "formality_level": 3,
                "key_phrases": ["that popular item", "saw on your website", "tell me more"]
            },
            product_references=[
                ProductReference(
        ...
        vague_signal = next((s for s in analysis["customer_signals"] 
                              if s["signal_type"] == "information_seeking"), None)
    ...
    async def test_missing_subject_email(self, mock_get_llm_client, mock_verify_analysis, mock_openai_client):
        ...
        browsing_intent = next((s for s in analysis["customer_signals"] 
                               if s["signal_type"] == "browsing_intent"), None)
    ...
    async def test_tangential_info_email(self, mock_get_llm_client, mock_verify_analysis, mock_openai_client):
        ...
        mock_output = EmailAnalysis(
            ...
            tone_analysis={
                "tone": "rambling",
                "formality_level": 2,
                "key_phrases": ["other messenger bag", "briefcase style options", "really nice leather briefcase"]
            },
            product_references=[
                ProductReference(
        ...
        irrelevant_signals = [s for s in analysis["customer_signals"] 
                             if s["signal_type"] == "irrelevant_information"]
        self.assertGreaterEqual(len(irrelevant_signals), 2)
        
        # Check for actual product inquiry signal
        inquiry_signal = next((s for s in analysis["customer_signals"] 
                              if s["signal_type"] == "researching_intent"), None)
