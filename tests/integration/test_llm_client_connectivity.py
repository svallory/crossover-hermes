    ...
    def test_successful_connection_with_custom_url(self):
        ...
        try:
            ...
            self.assertIsNotNone(
                config.llm_api_key,
                "API key should be configured either in script or via environment for this test.",
            )
            self.assertNotEqual(
                config.llm_api_key,
                "<OPENAI_API_KEY_PLACEHOLDER>",
                "Placeholder API key should be replaced for this test.",
            )
            ...
            self.assertIsInstance(
                llm,
                get_llm_client.__annotations__["return"],
                "LLM client is not of the expected type.",
            )
            ...
            self.assertTrue(
                hasattr(completion, "content"),
                "LLM response does not have 'content' attribute.",
            )
