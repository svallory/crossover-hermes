...
class TestProductResolver:
    ...
    @pytest.fixture
    def email_analyzer_output(self, exact_id_mention, fuzzy_name_mention):
        """Create an EmailAnalyzerOutput with multiple product mentions."""
        segment = Segment(
    ...
    @pytest.mark.asyncio
    async def test_full_resolver_agent(self, email_analyzer_output, hermes_config, vector_store):
        """Test the full resolution agent with multiple product mentions."""
        result = await resolve_product_mentions(
            state=email_analyzer_output,
            runnable_config={"configurable": {"hermes_config": hermes_config}}
        )
        
        assert result is not None, "Should return a result"
        assert Agents.PRODUCT_RESOLVER in result, "Result should contain the PRODUCT_RESOLVER key"
        
        # Cast to the correct type to satisfy type checker
        output = cast(Any, result[Agents.PRODUCT_RESOLVER])
        ...
        product_count = 0
        if email_analyzer_output.email_analysis and email_analyzer_output.email_analysis.segments:
            for segment in email_analyzer_output.email_analysis.segments:
                product_count += len(segment.product_mentions)
        assert output.metadata["resolution_attempts"] == product_count, "Should attempt to resolve all products in segments"
    ...
    async def test_disambiguation_prompt_construction(self, ambiguous_mention, hermes_config, vector_store):
        ...
        prompt_vars = {
            ...
            "product_category": ambiguous_mention.product_category.value if ambiguous_mention.product_category else "Not provided",
        ...
        disambiguation_prompt = get_prompt(f"{Agents.PRODUCT_RESOLVER}_disambiguation")
    ...
    async def test_ambiguous_resolution(self, ambiguous_mention, hermes_config, vector_store):
        ...
        if len(candidates) >= 2:
            ...
            # Cast to the correct type to satisfy type checker
            resolver_output = cast(Any, result[Agents.PRODUCT_RESOLVER])
    ...
    async def test_resolution_with_real_email_data(self, hermes_config, vector_store):
        ...
        assert len(resolver_output.resolved_products) >= exact_id_count * 0.75, "Should resolve most products with exact IDs"
    ...
    async def test_deduplicate_mentions_function(self, duplicate_product_mentions, hermes_config):
        ...
        deduplicated_mentions = await deduplicate_mentions(
            mentions=duplicate_product_mentions,
            email_context=email_context,
            hermes_config=hermes_config
        )
        ...
        assert all(isinstance(item, ProductMention) for item in deduplicated_mentions), "All items should be ProductMention objects"
        
        # The function should deduplicate, so we expect fewer items than the original
        assert len(deduplicated_mentions) < len(duplicate_product_mentions), "Deduplication should reduce the number of items"
    ...
    async def test_deduplication_in_resolver_agent(self, hermes_config, vector_store):
        ...
        assert resolver_output.metadata["total_mentions"] <= 2, "Should have 2 or fewer total mentions after deduplication" 