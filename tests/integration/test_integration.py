...
async def mock_analyze_email(state, config=None):
    ...
    is_order = any(term in email_body for term in ["order", "purchase", "buy", "ship", "pairs", "ordering"])
    ...
    for product_id in product_ids:
        ...
        quantity_match = re.search(
            r"(\d+)\s+(?:pairs? of\s+)?(?:.*?" + product_id + ")|(" + product_id + r".*?\s+(\d+))",
            email_body,
            re.IGNORECASE,
        )
        quantity = 1
...
async def mock_process_order(state, config=None):
    ...
    order_result = {"order_items": [], "order_total": 0.0, "order_status": "created"}
...
async def mock_process_inquiry(state, config=None):
    ...
    inquiry_result = {"product_info": [], "answers": []}
    ...
    for ref in product_references:
        ...
        inquiry_result["answers"].append({"question": state.email_body, "answer": answer, "confidence": 0.9})
        ...
        for item in items:
            ...
            if status == "created":
                response_parts.append(
                    f"We have processed your order for {item.get('quantity')} {item.get('product_name')}."
                )
                if item.get("promotion_details"):
                    response_parts.append(f"Special promotion: {item.get('promotion_details')}")
            elif status == "out_of_stock":
                response_parts.append(
                    f"We're sorry, but {item.get('product_name')} is out of stock for the requested quantity."
                )
...
class TestHermesPipeline(unittest.TestCase):
    ...
    def test_product_inquiry_flow(self):
        ...
        email_body = style_case.get(
            "email_body",
            "Hello, I'm interested in your Retro Sunglasses (RSG8901). What era are they inspired by?",
        )
