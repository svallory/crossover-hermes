...
def convert_notebook_to_markdown(ipynb_file_path: str, output_file_path: str | None = None) -> None:
    ...
    try:
        ...
        print(f"Error: Could not decode JSON from file: {ipynb_file_path}", file=sys.stderr)
        sys.exit(1)
...
if __name__ == '__main__':
    if not 2 <= len(sys.argv) <= 3:
        print(f"Usage: python3 {sys.argv[0]} <path_to_ipynb_file> [output_markdown_file_path]", file=sys.stderr)
        sys.exit(1)
