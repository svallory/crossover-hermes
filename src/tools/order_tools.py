...
def update_stock(product_id: str, quantity_to_decrement: int, product_catalog_df: pd.DataFrame = None) -> StockUpdateResult:
    ...
    # Find the product
    product_rows = product_catalog_df[product_catalog_df['product_id'] == product_id]
    ...
    product_name = product_catalog_df.loc[product_row_index, 'name']
    current_stock = int(product_catalog_df.loc[product_row_index, 'stock'])
    ...
    product_catalog_df.loc[product_row_index, 'stock'] = new_stock
...
def find_alternatives_for_oos(original_product_id: str, product_catalog_df: pd.DataFrame = None, limit: int = 2) -> Union[List[AlternativeProduct], ProductNotFound]:
    ...
    alternatives_df = product_catalog_df[
        (product_catalog_df['category'] == original_category) & 
        (product_catalog_df['product_id'] != original_product_id) &
        (product_catalog_df['stock'] > 0)
    ]
    ...
    for pattern in bogo_patterns:
        if re.search(pattern, description_lower):
            ...
            if not result.promotion_text:  # Don't overwrite if we already found a discount
                result.promotion_text = extract_promotion_text(product_description, re.search(pattern, description_lower))
            break
