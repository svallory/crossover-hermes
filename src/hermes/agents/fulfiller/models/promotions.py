...
class DiscountSpec(BaseModel):
    ...
    amount: float = Field(
        gt=0, 
        description="Amount of discount (percentage or fixed amount)"
    )
...
class PromotionEffects(BaseModel):
    ...
    apply_discount: Optional[DiscountSpec] = Field(
        default=None,
        description="Specification for a discount to apply"
    )
