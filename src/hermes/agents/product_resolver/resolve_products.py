...
from typing import Optional, Dict, Any, Literal, List, Tuple, Union, cast
...
def resolve_product_reference(
    ...
    if "name" in query and query["name"]:
        name_matches = find_products_by_name(
            name=query["name"],
...
async def run_deduplication_llm(
    ...
    deduplication_prompt = get_prompt(f"{Agents.PRODUCT_RESOLVER}_deduplication")
    ...
    if deduplicated_data:
        ...
        for item in deduplicated_data:
            ...
            mention = ProductMention(
                ...
                product_description=item.get("product_description") if item.get("product_description") != "Not provided" else None,
...
async def resolve_product_mentions(
...
) -> WorkflowNodeOutput[Literal[Agents.PRODUCT_RESOLVER], ResolvedProductsOutput]:
    """
    ...
    try:
        ...
        return create_node_response(Agents.PRODUCT_RESOLVER, output)
        
    except Exception as e:
        return create_node_response(Agents.PRODUCT_RESOLVER, e) 