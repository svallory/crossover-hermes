...
- A `retrieved_products_context` string containing product information potentially relevant to the customer's inquiry, retrieved from our product catalog based on their questions and mentions.
...
3.  `answered_questions`: Questions from the customer with factual answers, formulated by cross-referencing the `retrieved_products_context`.
4.  `related_products`: Objectively relevant products based on customer inquiries, supported by information in `retrieved_products_context`.
5.  `unanswered_questions`: Questions that couldn't be answered even with the `retrieved_products_context`.
6.  `unsuccessful_references`: Product references that couldn't be resolved against the `retrieved_products_context` or internal knowledge.
...
2. If a question mentions products that aren't detailed in the `retrieved_products_context`, note this in `unanswered_questions`.
3. If a question requires information not available in the `retrieved_products_context`, note this in `unanswered_questions`.
4. For questions about general product features, provide neutral, objective information, ideally supported by `retrieved_products_context`.
5. Populate the `answered_questions` field with QuestionAnswer objects for questions you can answer using the provided context.
6. Set appropriate confidence levels based on how well the `retrieved_products_context` supports the answer.

FOURTH TASK - RELATED PRODUCTS IDENTIFICATION (USING RETRIEVED CONTEXT):
1. Based on the customer's inquiries and mentioned products, and supported by the `retrieved_products_context`, identify objectively related products.
2. Focus on functional relationships (same category, complementary items) found within the `retrieved_products_context`.
3. Include only products that have a clear, factual relationship to the customer's interests, as evidenced by the context.

IMPORTANT NOTES:
- Provide ONLY factual, objective information without subjective language, based on the `retrieved_products_context` where possible.
