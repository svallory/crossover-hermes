...
from src.hermes.model.order import OrderLine, Order
from src.hermes.model.promotions import PromotionSpec


@tool
def apply_promotion(tool_input: str) -> Order:
    ...
    Args:
        tool_input: JSON string with ordered_items, promotion_specs, and email_id fields.
    ...
    try:
        ...
        ordered_items = input_data.get("ordered_items", [])
        promotion_specs_data = input_data.get("promotion_specs", [])
        email_id = input_data.get("email_id", "unknown")
    except (json.JSONDecodeError, AttributeError):
        # Return a minimal valid Order if there's an error
        return Order(
            email_id="error",
    ...
    # Convert promotion_specs_data to PromotionSpec objects if they aren't already
    promotion_specs = []
    for spec_data in promotion_specs_data:
        if isinstance(spec_data, dict):
            try:
                promotion_specs.append(PromotionSpec(**spec_data))
            except Exception:
                # Skip invalid promotion specs
                pass
        elif isinstance(spec_data, PromotionSpec):
            promotion_specs.append(spec_data)
    ...
    for item in ordered_items:
        ...
        for promotion in promotion_specs:
            # Check min_quantity condition
            if (promotion.conditions.min_quantity is not None and 
                quantity >= promotion.conditions.min_quantity):
                
                # Apply percentage discount if specified
                if (promotion.effects.apply_discount is not None and
                    (promotion.effects.apply_discount.to_product_id is None or
                     promotion.effects.apply_discount.to_product_id == product_id)):
                    ...
                    if discount_spec.type == "percentage":
                        ...
                        promotion_description = f"{discount_spec.amount}% discount applied"
                    elif discount_spec.type == "fixed":
                        discount_amount = min(discount_spec.amount, unit_price)  # Cannot discount below zero
                        unit_price -= discount_amount
                        total_discount += discount_amount * quantity
                        promotion_applied = True
                        promotion_description = f"${discount_spec.amount} discount applied"
                ...
                if promotion.effects.free_items is not None:
                    ...
                    if effective_quantity > 0:
                        discount_per_item = unit_price * (free_count / effective_quantity)
                ...
                if promotion.effects.free_gift is not None:
                    ...
                    promotion_description = f"Free gift: {gift_description}" if promotion_description is None else f"{promotion_description} + Free gift: {gift_description}"
